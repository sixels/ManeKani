...
func (cq *CardQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (cq *CardQuery) loadReviews(ctx context.Context, query *ReviewQuery, nodes []*Card, init func(*Card), assign func(*Card, *Review)) error {
	...
	query.Where(predicate.Review(func(s *sql.Selector) {
		s.Where(sql.InValues(card.ReviewsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "card_reviews" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
