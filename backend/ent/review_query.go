...
func (rq *ReviewQuery) QueryCard() *CardQuery {
...
func (rq *ReviewQuery) WithCard(opts ...func(*CardQuery)) *ReviewQuery {
	query := &CardQuery{config: rq.config}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withCard; query != nil {
		if err := rq.loadCard(ctx, query, nodes, nil,
			func(n *Review, e *Card) { n.Edges.Card = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewQuery) loadCard(ctx context.Context, query *CardQuery, nodes []*Review, init func(*Review), assign func(*Review, *Card)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Review)
	for i := range nodes {
		if nodes[i].card_reviews == nil {
			continue
		}
		fk := *nodes[i].card_reviews
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "card_reviews" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
