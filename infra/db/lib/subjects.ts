		...
		const foundSubjects = await inlineAsyncTry(
			...
			(error) => {
				throw PrismaErrors.match(error, {
					fallback: {
						description:
							"An unknown error occurred while retrieving the subjects.",
						context: { filters },
					},
				});
			},
					...
					byError: {
						[PrismaErrors.UniqueConstraint]: [
							ResourceCollidesError,
							{
								context: errorContext,
								description:
									"A subject with same slug and category already already exists in this deck.",
							},
						],
						...
						[PrismaErrors.DependencyNotFound]: [
							InvalidRequestError,
							{
								context: errorContext,
								description:
									"The subject depends on a subject which does not exist. check if all dependencies, dependents or similar subjects exist.",
							},
						],
					},
				});
					...
					byError: {
						...
						[PrismaErrors.DependencyNotFound]: [
							InvalidRequestError,
							{
								context: errorContext,
								description:
									"The subject or one of its dependencies does not exist. check if all dependencies, dependents or similar subjects exist.",
							},
						],
					},
				});
