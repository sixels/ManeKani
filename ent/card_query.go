...
func (cq *CardQuery) QueryDeckProgress() *DeckProgressQuery {
	query := (&DeckProgressClient{config: cq.config}).Query()
...
func (cq *CardQuery) QuerySubject() *SubjectQuery {
...
func (cq *CardQuery) QueryReviews() *ReviewQuery {
	query := (&ReviewClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CardQuery) WithSubject(opts ...func(*SubjectQuery)) *CardQuery {
	query := (&SubjectClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CardQuery) WithReviews(opts ...func(*ReviewQuery)) *CardQuery {
	query := (&ReviewClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withReviews = query
	return cq
}
...
func (cq *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := cq.withSubject; query != nil {
		if err := cq.loadSubject(ctx, query, nodes, nil,
			func(n *Card, e *Subject) { n.Edges.Subject = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withReviews; query != nil {
		if err := cq.loadReviews(ctx, query, nodes,
			func(n *Card) { n.Edges.Reviews = []*Review{} },
			func(n *Card, e *Review) { n.Edges.Reviews = append(n.Edges.Reviews, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CardQuery) loadSubject(ctx context.Context, query *SubjectQuery, nodes []*Card, init func(*Card), assign func(*Card, *Subject)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Card)
	for i := range nodes {
		if nodes[i].subject_cards == nil {
			continue
		}
		fk := *nodes[i].subject_cards
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subject_cards" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CardQuery) loadReviews(ctx context.Context, query *ReviewQuery, nodes []*Card, init func(*Card), assign func(*Card, *Review)) error {
	...
	query.Where(predicate.Review(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(card.ReviewsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.card_reviews
		if fk == nil {
			return fmt.Errorf(`foreign-key "card_reviews" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "card_reviews" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
