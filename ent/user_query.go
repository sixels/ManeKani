...
func (uq *UserQuery) QueryDecks() *DeckQuery {
	query := (&DeckClient{config: uq.config}).Query()
...
func (uq *UserQuery) QueryDecksProgress() *DeckProgressQuery {
	query := (&DeckProgressClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) loadSubjects(ctx context.Context, query *SubjectQuery, nodes []*User, init func(*User), assign func(*User, *Subject)) error {
	...
	query.Where(predicate.Subject(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SubjectsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_subjects" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadSubscribedDecks(ctx context.Context, query *DeckQuery, nodes []*User, init func(*User), assign func(*User, *Deck)) error {
	...
	neighbors, err := withInterceptors[[]*Deck](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (uq *UserQuery) loadDecksProgress(ctx context.Context, query *DeckProgressQuery, nodes []*User, init func(*User), assign func(*User, *DeckProgress)) error {
	...
	query.Where(predicate.DeckProgress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DecksProgressColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_decks_progress" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
