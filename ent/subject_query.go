...
func (sq *SubjectQuery) QueryDeck() *DeckQuery {
	query := (&DeckClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubjectQuery) loadCards(ctx context.Context, query *CardQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *Card)) error {
	...
	query.Where(predicate.Card(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subject.CardsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subject_cards" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
