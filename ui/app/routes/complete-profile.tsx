...
export default function Component() {
	...
	return (
		<main className="flex max-w-screen-2xl md:p-2.5 gap-2.5 overflow-hidden h-screen mx-auto">
			...
			<div className="bg-neutral-200 h-full rounded-xl w-full max-w-xs lg:max-w-sm hidden md:block" />
		</main>
	);
}
...
function handleValidationError(error: InvalidRequestError) {
	...
	if (error.context instanceof Object && "errors" in error.context) {
		const {
			...
			// biome-ignore lint/suspicious/noExplicitAny: We don't have the exact context type, so this helps with the parsing
