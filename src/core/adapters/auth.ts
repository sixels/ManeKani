...
import { ISsoAuthenticator, ITokenAuthenticator } from '../ports/auth';
...
export class SsoAuthAdapter<R extends ISsoAuthenticator> {
  ...
  async getSessionFromCookies(cookies: string): Promise<UserSession> {
    const session = await this.ssoProvider.getSessionFromCookies(cookies);
...
export class TokenAuthAdapter<R extends ITokenAuthenticator> {
  ...
  async getSessionFromToken(
    ...
    if (!hasRequiredScopes(userToken, requiredScopes)) {
      throw new ForbiddenError({
        ...
        description: `The provided token does not have enough permissions to perform this action. The required permissions are: ${requiredScopes.join(
          ', ',
        )}`,
      });
    }

    return { id: userToken.ownerId };
  }
}
...
function hasRequiredScopes(token: Token, requiredScopes: string[]): boolean {
  ...
  return requiredScopes.every((scope) => Boolean(claimsMap[scope]));
}
