...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request: Request & { user?: UserSession } = context
      .switchToHttp()
      .getRequest();
...
async function authorizeApiToken(
  tokenProvider: TokenAuthAdapter<ITokenAuthenticator>,
  request: Request,
...
async function authorizeLogin(
  ...
  return await ssoProvider.getSessionFromCookies(loginCookies);
}
